...
- **State diffing in the DB** — Slot availability is stored per-check, so the system can detect transitions (unavailable → available) reliably across restarts without holding state in memory.
